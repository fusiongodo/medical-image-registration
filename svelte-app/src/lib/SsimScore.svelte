<script lang="ts">
	...
	import { normalizeImageData, computeSSIM, shiftGray } from '$lib/imageUtils';
	...
	$effect(() => {
		if (displaced && !hasDisplacement) return;
		const g1 = gray1, g2 = gray2, w = imgW, h = imgH, ps = patchSize;
		...
		const handle = requestIdleCallback(() => {
			const g2final = (ddx !== undefined && ddy !== undefined)
				...
				: g2;
			const s = computeSSIM(g1, g2final, w, h, ps);
			score = s;
			untrack(() => onscore?.(s));
		});
