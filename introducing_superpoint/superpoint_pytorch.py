...
class SuperPoint(nn.Module):
    ...
    def forward(self, data, training=False):
        """
        data["image"]: [B, 1 or 3, H, W] float
        training=False -> inference dict (keypoints, keypoint_scores, descriptors)
        training=True  -> {
            "logits":          [B, 65,  H/8, W/8]  raw detector head, pre-softmax
            "descriptors_raw": [B, 256, H/8, W/8]  raw descriptor head, pre-L2-norm
        }
        """
        image = data["image"]
        ...
        if training:
            return {
                ...
                "descriptors_raw": self.descriptor(features),
            }
